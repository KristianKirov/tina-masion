...
class WCPBC_Admin_Product_Data {
	...
	public static function variable_product_bulk_edit_actions(){
		...
		$variable_actions = array(
			'wcpbc_variable_regular_price' => __( 'Set regular prices', 'woocommerce' ),
			'wcpbc_variable_regular_price_increase' => __( 'Increase regular prices (fixed amount or percentage)', 'woocommerce' ),
			'wcpbc_variable_regular_price_decrease' => __( 'Decrease regular prices (fixed amount or percentage)', 'woocommerce' ),
			'wcpbc_variable_sale_price' => __( 'Set sale prices', 'woocommerce' ),
			'wcpbc_variable_sale_price_increase' => __( 'Increase sale prices (fixed amount or percentage)', 'woocommerce' ),
			'wcpbc_variable_sale_price_decrease' => __( 'Decrease sale prices (fixed amount or percentage)', 'woocommerce' )
		);
		...
		foreach ( WCPBC()->get_regions() as $zone_id => $zone ) {
			...
			foreach ($variable_actions as $key => $label ) {
				echo '<option value="' . $key . '" data-zone-id="' . $zone_id . '">' . $label . '</option>';
			}		
			echo '</optgroup>';
		}		
	...
	public static function bulk_edit_variations( $bulk_action, $data, $product_id, $variations ) {		
		...
		$wcpbc_actions = array('wcpbc_variable_regular_price', 'wcpbc_variable_regular_price_increase', 'wcpbc_variable_regular_price_decrease', 'wcpbc_variable_sale_price', 'wcpbc_variable_sale_price_increase', 'wcpbc_variable_sale_price_decrease');
		if ( !in_array($bulk_action, $wcpbc_actions) || empty( $data['zone_id'] ) || ! array_key_exists( $data['zone_id'], WCPBC()->get_regions() ) ) {
			return;
		}
		...
		foreach ( $variations as $variation_id ) {
			...
			if ($price_method != 'manual') {
				update_post_meta( $variation_id, $meta_prefix . '_price_method', 'manual');
			}
			
			switch ($bulk_action) {
				case "wcpbc_variable_regular_price":
					update_post_meta($variation_id, $meta_prefix . '_regular_price', $value);
					break;
				case "wcpbc_variable_regular_price_increase":
					WCPBC_Admin_Product_Data::update_variation_price($variation_id, $meta_prefix . '_regular_price', '+', wc_clean($value));
					break;
				case "wcpbc_variable_regular_price_decrease":
					WCPBC_Admin_Product_Data::update_variation_price($variation_id, $meta_prefix . '_regular_price', '-', wc_clean($value));
					break;
				case "wcpbc_variable_sale_price":
					update_post_meta($variation_id, $meta_prefix . '_sale_price', $value);
					break;
				case "wcpbc_variable_sale_price_increase":
					WCPBC_Admin_Product_Data::update_variation_price($variation_id, $meta_prefix . '_sale_price', '+', wc_clean($value));
					break;
			...
			$new_price = get_post_meta($variation_id, $meta_prefix . '_sale_price', true);
			if (!is_numeric($sale_price)) {
				$new_price = get_post_meta($variation_id, $meta_prefix . '_regular_price', true);
			}
			update_post_meta($variation_id, $meta_prefix . '_price', $new_price);
		}
	}

	private static function update_variation_price($variation_id, $field_name, $operator, $value) {
		$price = get_post_meta($variation_id, $field_name, true);
		if ('%' === substr($value, -1)) {
		...
		update_post_meta($variation_id, $field_name, $price);
	}
}
