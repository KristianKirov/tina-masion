...
function wcpbc_get_woocommerce_country() {
	...
	if ( wcpbc_is_woocommerce_frontend() ) {
		$country_detection_option = get_option('wc_price_based_country_based_on', 'billing');
		if ($country_detection_option === 'first_detected') {
			$initial_country = WC()->session->get('customer_initial_country');
			if (!empty($initial_country)) {
				$_country = $initial_country;
			}
		}
		...
		if (!$_country) {
			...
			if ( $_country !== WC()->customer->get_shipping_country() && 'shipping' === $country_detection_option) {
